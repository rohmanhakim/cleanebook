...
**svelte-kit sync before tests:**
The `tsconfig.json` extends `.svelte-kit/tsconfig.json`, which is generated by SvelteKit. In CI, the `.svelte-kit/` directory doesn't exist (it's in `.gitignore`). The `unit-tests` job must run `svelte-kit sync` before `pnpm test` to generate this file, otherwise TypeScript resolution fails with:

```
TSConfckParseError: failed to resolve "extends":"./.svelte-kit/tsconfig.json"
```

The `lint-and-typecheck` job doesn't have this issue because `pnpm check` runs `svelte-kit sync` as part of the check script.
